...
<?php
	...
	function get_columns() {
		$c = array(
			...
			'email'    => __( 'E-mail' ),
			'role'     => __( 'Role' ),
			'listings'    => __( 'Listing Alerts', 'list-alerts' )
		);
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			switch ( $column_name ) {
				...
				case 'listings':
					...
					$r .= "<td $attributes>";
					if ( $numposts >= 0 ) {
						$r .= "<a href='" . add_query_arg(array('la_user' => $user_object->ID), $this->la_user_list_url()) . "' title='" . esc_attr__( 'View & edit listings by this user', 'list-alerts' ) . "' class='edit'>";
						$r .= __('Show alert criteria', 'list-alerts');
						$r .= '</a>';
					} else {
						$r .= 0;
					}
				...
				default:
					$r .= "<td $attributes>";
					$r .= apply_filters( 'manage_admin_page_list_alerts_users_custom_column', '', $column_name, $user_object->ID );
					$r .= "</td>";
			}
		}
		$r .= '</tr>';
