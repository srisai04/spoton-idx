...
jQuery(document).ready(function(){
    ...
    if((jQuery.cookie('spot-on') != null)||(jQuery.cookie('spot-ln')!= null))
    { 
         getSearchState();
    }
	  ...
	  <div class="box_c_heading cf">
		...
		<div class="box_c_content cf tabs_content">
			...
			<div>
                ...
                <!--.replace("mapinfoboximages","retsmapthumbnailimages")-->
...
<div>  
  ...
  <div class="mAccordion box_c_content">
	...
	<div class="micro">
		...
		<div class="sub_section cf">   
			<label>MLS #&nbsp;</label>
			<input style="width:140px" type="text" id="spto_mls1" value=""/><p/>    
            <label>MLS #&nbsp;</label>
			<input style="width:140px" type="text" id="spto_mls2" value=""/><p/>  
            <label>MLS #&nbsp;</label>
			<input style="width:140px" type="text" id="spto_mls3" value=""/><p/>  
            <label>MLS #&nbsp;</label>
			<input style="width:140px" type="text" id="spto_mls4" value=""/><p/>  
            <label>MLS #&nbsp;</label>
			<input style="width:140px" type="text" id="spto_mls5" value=""/><p/>  
            <label>MLS #&nbsp;</label>
			<input style="width:140px" type="text" id="spto_mls6" value=""/><p/>
...
<?php } ?>
		...
		{     
            jQuery.cookie('spot-on',null,{ expires: -1,path: '/' });
        ...
        var search_agn='<?php echo spoton_list_hide("search_agent") ?>';
         ...
         if(search_agn == 1)
			...
			if(multiLN.length!=0 && submitClick==2)
				...
				requestUri = serviceUri + "GenericRetsSearchListings?" + query +"$select=PKey,LN,HouNo,DirPre,Stre,StrSuff,DirSuff,City,State,Zip,County,PropType,SquFeet,Price,Acre,Beds,Baths,CreDate,ImgNam,"+AgName+"Logo,"+offname+"AddrYN&$inlinecount=allpages";
			else    
				requestUri = serviceUri + "GenericRetsSearchListings?" + query +"$select=PKey,LN,HouNo,DirPre,Stre,StrSuff,DirSuff,City,State,Zip,County,PropType,SquFeet,Price,Acre,Beds,Baths,CreDate,ImgNam,"+AgName+"Logo,"+offname+"AddrYN&" + "$skip="+String(no)+"&$top=50&$inlinecount=allpages";     
		...
		{	
			...
			OData.defaultHttpClient.enableJsonpCallback = true;
			OData.read(requestUri, function (data, request) {
            var disclaimer="";var LUD="";var updatetime=""; var suffix = "AM";
				for (var i = 0; i < data.results.length; i++) 
				{	
